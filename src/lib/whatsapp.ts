...
/**
 * Creates an executive, well-formatted WhatsApp message tailored for Izzy Immigration advisors
 */
export function formatWhatsAppMessage(data: WhatsAppEnquiryData): string {
  const parts: string[] = [];

  // Header
  let header = "🏛️ *IZZY IMMIGRATION — PRIVATE CLIENT ENQUIRY*";
  if (data.type === "consultation") {
    header = "🏛️ *IZZY IMMIGRATION — CONSULTATION BOOKING*";
  } else if (data.type === "brochure") {
    header = "🏛️ *IZZY IMMIGRATION — OFFICIAL DOSSIER REQUEST*";
  } else if (data.type === "ancestry") {
    header = "🏛️ *IZZY IMMIGRATION — ANCESTRY CITIZENSHIP CHECK*";
  } else if (data.type === "matcher") {
    header = "🏛️ *IZZY IMMIGRATION — SOVEREIGN ROUTE ASSESSMENT*";
  } else if (data.type === "inquiry") {
    header = "🏛️ *IZZY IMMIGRATION — STATUTORY PROGRAMME INQUIRY*";
  }
